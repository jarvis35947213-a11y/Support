...
FAQ_FILE = os.path.join(_ROOT, "frontend", "faq.json")
...
def _build_system_prompt():
    ...
    base = (
        ...
        "Ниже история переписки с пользователем.\n\n"
    )
    if faq_text:
        base += (
            "Вот официальная база знаний (FAQ) с готовыми ответами на частые проблемы. "
            "Найди в ней пункты, соответствующие проблеме пользователя, и используй их для ответа. "
            "Можешь процитировать номер пункта (например, «Следуйте пункту 4.0»). "
            "Не выдумывай того, чего нет в FAQ.\n\n"
            f"{faq_text}\n\n"
        )
    base += (
        "Напиши ВЕЖЛИВЫЙ и ПОЛЕЗНЫЙ ответ от лица поддержки на РУССКОМ языке. "
        "ОБЯЗАТЕЛЬНО: если в FAQ есть пункт, соответствующий проблеме пользователя, "
        "процитируй его номер (например, «Пункт 2.0») и дай ответ из этого пункта. "
